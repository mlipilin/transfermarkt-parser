...
export function list (clubId, seasonId): Promise<Array<iPlayer>> {
    return axios.get(url.player.list(clubId, seasonId))
        .then(response => {
            ...
            return [...dom.window.document.querySelectorAll('#yw1 table.items > tbody > tr')]
                ...
                .map(node => {
                    ...
                    const logoUrl = logoNode && logoNode.src ? logoNode.src.replace('small', 'big') : null;
                    ...
                    const birthday = birthdayNode ? moment(birthdayNode.innerHTML, 'MMM DD, YYYY').format('YYYY-MM-DD') : null;
